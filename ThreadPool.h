        ...
        while (!done)
        {
            ...
            if (queues[i]->TryPop(task))
            {
                ...
                for (int child_c = 0; child_c < child_threads_count; child_c++) {
                    ...
                    child_threads.emplace_back(threadFunction, task.Path, child_c, child_threads_count, std::move(promises.back()));  // ¬изиваЇмо функц≥ю
                }
                // Retrieve results from futures
                for (int fut = 0; fut < child_threads_count; ++fut) {
                    std::unordered_map<std::string, std::vector<std::string>> result = futures[fut].get(); // отримуЇмо дан≥ з future
